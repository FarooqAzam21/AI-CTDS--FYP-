...
def _resolve_jwt(token: str, db: Session, request: Request) -> AuthContext:
    ...
    try:
        ...
        try:
            ...
            # Do not update login_count here. This dependency runs for every
            # authenticated API request (dashboard cards, alerts, scans, etc.),
            # not just at sign-in. Federated sign-ins are recorded once by
            # POST /api/v1/me/record-login after Supabase emits SIGNED_IN.
